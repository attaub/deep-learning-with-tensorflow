...
load it, then split it into a training set, a validation set and a test set
finally we scale it
"""
...
split it into many files first
then have TensorFlow read these files in parallel
split the housing dataset and save it to 20 CSV files
"""
...
def save_to_multiple_csv_files(data, name_prefix, header=None, n_parts=10):
    ...
    for file_idx, row_indices in enumerate(
        ...
        with open(part_csv, "wt", encoding="utf-8") as f:
            ...
            for row_idx in row_indices:
                f.write(
                    ",".join([f"{float(col):.6f}" for col in data[row_idx]])
...
################################################################
# take a peek at the first few lines of one of these CSV files
...
# 5th field is compulsory (tf.constant([])), an exception if we do not provide it:
...
# number of fields should match the number of fields in the record_defaults
try:
...
@tf.function
def preprocess(line):
    defs = [tf.constant(0.0, dtype=tf.float32)] * n_inputs + [
        tf.constant([], dtype=tf.float32)
    ]
    fields = tf.io.decode_csv(line, record_defaults=defs)
    x = tf.stack(fields[:-1])
    y = tf.stack(fields[-1:])
    return (x - tf.constant(X_mean, dtype=tf.float32)) / tf.constant(
        X_std, dtype=tf.float32
    ), y


line_ = b'4.2083,44.0,5.3232,0.9171,846.0,2.3370,37.47,-122.2,2.782'
...
#################################################################
#################################################################
"""# The TFRecord binary format: file is just a list of binary records
...
"""# Read multiple TFRecord files with just one TFRecordDataset
(default is one) set num_parallel_reads=3 will read 3 at in parallel
and interleave their records """
...
Install protobuf, write a simple protobuf definition: """
...
#################################################################
""" compile it (--descriptor_set_out and --include_imports options are only required for the tf.io.decode_proto() example below)
"""
...
person_example = Example(
    features=Features(
        feature={
            ...
            "emails": Feature(
                bytes_list=BytesList(value=[b"a@b.com", b"c@d.com"])
            ),
        }
    )
)
...
for serialized_examples in dataset:
    parsed_examples = tf.io.parse_example(
        serialized_examples, feature_description
    )
parsed_examples
#################################################################
#################################################################
...
#################################################################
"""# Warning: In TensorFlow 2.0 and 2.1 there was a bug
preventing from tensorflow.train import X
...
bucketized_income = tf.feature_column.bucketized_column(
    median_income, boundaries=[1.5, 3.0, 4.5, 6.0]
)
bucketized_income
...
bucketized_age = tf.feature_column.bucketized_column(
    housing_median_age, boundaries=[-1.0, -0.5, 0.0, 0.5, 1.0]
)  # age was scaled
...
bucketized_latitude = tf.feature_column.bucketized_column(
    latitude, boundaries=list(np.linspace(32.0, 42.0, 20 - 1))
)
bucketized_longitude = tf.feature_column.bucketized_column(
    longitude, boundaries=list(np.linspace(-125.0, -114.0, 20 - 1))
)
...
with tf.io.TFRecordWriter("my_data_with_features.tfrecords") as f:
    for x, y in zip(X_train[:, 1:2], y_train):
        example = Example(
            features=Features(
                feature={
                    "housing_median_age": Feature(
                        float_list=FloatList(value=[x])
                    ),
                    "median_house_value": Feature(
                        float_list=FloatList(value=[y])
                    ),
                }
            )
        )
...
try:
    ...
    def preprocess(inputs):  # inputs is a batch of input features
        ...
        standardized_age = tft.scale_to_z_score(
            median_age - tft.mean(median_age)
        )
...
def create_example(image, label):
    ...
    return Example(
        features=Features(
            feature={
                "image": Feature(
                    bytes_list=BytesList(value=[image_data.numpy()])
                ),
...
def write_tfrecords(name, dataset, n_shards=10):
    ...
    with ExitStack() as stack:
        writers = [
            stack.enter_context(tf.io.TFRecordWriter(path)) for path in paths
        ]
...
logs = os.path.join(
    os.curdir, "my_logs", "run_" + datetime.now().strftime("%Y%m%d_%H%M%S")
)
tensorboard_cb = tf.keras.callbacks.TensorBoard(
    log_dir=logs, histogram_freq=1, profile_batch=10
)
model.fit(
    train_set, epochs=5, validation_data=valid_set, callbacks=[tensorboard_cb]
)
...
get_ipython().run_line_magic(
    'timeit',
    '-r1 for X, y in imdb_dataset(train_pos, train_neg).repeat(10): pass',
)
...
get_ipython().run_line_magic(
    'timeit',
    '-r1 for X, y in imdb_dataset(train_pos, train_neg).cache().repeat(10): pass',
)
batch_size = 32
train_set = (
...
sample_reviews = np.concatenate(
    list(sample_review_batches.as_numpy_iterator()), axis=0
)
text_vectorization = TextVectorization(
    max_vocabulary_size, n_oov_buckets, input_shape=[]
)
...
model = keras.models.Sequential(
    [
        ...
        keras.layers.Embedding(
            input_dim=n_tokens, output_dim=embedding_size, mask_zero=True
