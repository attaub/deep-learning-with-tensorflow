...
# create a dataset (0 to 14), treat each integer as a separate element
dataset = tf.data.Dataset.from_tensor_slices(tf.range(15))

# Splits into windows of size=n_steps, shift by 2 elements
dataset = dataset.window(n_steps, shift=2, drop_remainder=True)

# convert each window (a sub-dataset) into a single batch tensor of n_steps elements.
dataset = dataset.flat_map(lambda window: window.batch(n_steps))

# shuffle the windows and maps each one to a tuple (inputs, targets)
dataset = dataset.shuffle(10).map(lambda window: (window[:-1], window[1:]))

# Batche the windows into groups of 3 and prefetches 1 batch
...
#################################################################
"""#Note: In previous versions of this code, we used dataset.repeat()
now to make the dataset "infinite", and later in the notebook we set the steps_per_epoch argument when calling the model.fit() method. This was needed to work around some TensorFlow bugs. However, since these bugs have now been fixed, we can simplify the code: no need for dataset.repeat() or steps_per_epoch anymore.
"""
...
#################################################################
import tensorflow_datasets as tfds  # pip3 install tensorflow_datasets


# Load the IMDb reviews dataset; return both data and metadata
datasets, info = tfds.load("imdb_reviews", as_supervised=True, with_info=True)

datasets.keys()  # get dictionary keys of dataset splits (e.g., 'train', 'test')

# get number of examples in training and testing sets
train_size = info.splits["train"].num_examples
test_size = info.splits["test"].num_examples

print(train_size, test_size)

# preview a single batch (2 samples) from the training data
...
# define a preprocessing function for cleaning and tokenizing text
def preprocess(X_batch, y_batch):
    # Truncate each review to the first 300 characters (to limit input size)
    X_batch = tf.strings.substr(X_batch, 0, 300)

    # Replace HTML line breaks like <br /> or <br> with a space
    X_batch = tf.strings.regex_replace(X_batch, rb"<br\s*/?>", b" ")

    # Replace all characters that are not letters or apostrophes with a space
    # This removes numbers, punctuation, etc.
    X_batch = tf.strings.regex_replace(X_batch, b"[^a-zA-Z']", b" ")

    # split each review into a list of words (tokens) using space as delimiter
    X_batch = tf.strings.split(X_batch)

    # pad all sequences in the batch to the same length with "<pad>" tokens
    return X_batch.to_tensor(default_value=b"<pad>"), y_batch


preprocess(X_batch, y_batch)

# Import Counter to build a vocabulary of word frequencie
from collections import Counter

# initialize an empty Counter object to store word frequencies
vocabulary = Counter()

# loop through the training dataset in batches of 32, applying preprocessing
for X_batch, y_batch in datasets["train"].batch(32).map(preprocess):

    # loop through each review in the batch
    for review in X_batch:

        # convert the review tensor to a list of bytes and update word counts
        vocabulary.update(list(review.numpy()))

# display the top 3 most common words in the dataset
vocabulary.most_common()[:3]

len(vocabulary)

# set the maximum vocabulary size to 10,000 words
vocab_size = 10000

# ctruncate vocabulary list containing only the top 10,000 most frequent words
truncated_vocabulary = [
    word for word, count in vocabulary.most_common()[:vocab_size]
]

# create a dictionary mapping each word in the truncated vocabulary to a unique index
word_to_id = {word: index for index, word in enumerate(truncated_vocabulary)}

# test the word-to-index dictionary on a sample sentence
for word in b"This movie was faaaaaantastic".split():

    # print the index of each word if it's in the dictionary
    # otherwise print vocab_size (indicates unknown word)
    print(word_to_id.get(word) or vocab_size)

# convert the truncated vocabulary list into a TensorFlow constant
words = tf.constant(truncated_vocabulary)

# create a range of integer IDs (0 to vocab_size - 1) for each word
word_ids = tf.range(len(truncated_vocabulary), dtype=tf.int64)

# initialize a lookup table that maps each word to its corresponding ID
vocab_init = tf.lookup.KeyValueTensorInitializer(words, word_ids)

# set the number of "out-of-vocabulary" (OOV) buckets to handle unknown words

num_oov_buckets = 1000

# create a static vocabulary table with OOV handling using the initializer and OOV bucket count

table = tf.lookup.StaticVocabularyTable(vocab_init, num_oov_buckets)

# use the table to look up word IDs for a test sentence (splits into words first)
table.lookup(tf.constant([b"This movie was faaaaaantastic".split()]))

# define a function to convert tokenized batches to their corresponding word IDs
...
# Create a dataset pipeline: batch the training set and apply the preprocess function
train_set = datasets["train"].batch(32).map(preprocess)

# apply the encode_words function to convert words to IDs and prefetch for performance
train_set = train_set.map(encode_words).prefetch(1)

# Take and print one batch from the transformed training set
for X_batch, y_batch in train_set.take(1):
    print(X_batch)  # print the batch of encoded word ID sequences
    print(y_batch)  # print the corresponding labels (0 or 1)


# set the size of the word embedding vectors to 128 dimensions
embed_size = 128
