...
"""
Find the top k words to start the translations.
For each of the top k translations, evaluate the conditional probabilities of
all possible words it could add to that translation.
These extended translations and their probabilities are added to the list of candidates
Once we have gone through all top k translations and all words that could complete them
we keep only the top k candidates with the highest probability,
and we iterate over and over until they all finish with an EOS token.
The top translation is then returned (after removing its EOS token).
